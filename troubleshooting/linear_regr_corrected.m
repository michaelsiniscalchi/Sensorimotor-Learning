function [ output ] = linear_regr_corrected ( signal, t, event, eventTime, trialSubset, params)
% % linear_regr %
...
%         end
        ...
        % terms = [terms zeros(size(terms,1),1)];   % Ver.2: terms matrix edited from original version
        % terms = y ~ 1 + x1*x4 + x2*x5 + x3*x6     % Ver.3: the (equivalent) Wilkinson notation
