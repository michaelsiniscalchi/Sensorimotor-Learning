function [ regData, modCells, params_out ] = calc_mlrChoiceOutcome(trigTimes, trials, cells, params)
...
for i = 1:size(name_value,1)
    ...
    factorChoice = false(size(trials.left)); %Initialize
    ...
    for j = cellList 
        ...
        regData(j) = linear_regr(cells.dFF{j}, cells.t,...
            [factorChoice factorOutcome], trigTimes, trialMask, params_out);
    end
