function [ cells ] = calc_dFF( stackInfo, cells, logfileTimes, timeLastEvent, corrFactor )
% % calc_dFF %
...
if nROIs > 0  %if there were any cells
    ...
    for i = 1:nROIs
        ...
        if corrFactor > 0   %if we are correcting for neuropil
            if all(~isnan(cells.neuropilf{i}))    %if neuropil data was extracted, and not NaN
                neuropilf = cells.neuropilf{i}(:);
                f = f - corrFactor * neuropilf;
            else
                warning('Could not correct for neuropil: signal is NaN.');
            end
        end
